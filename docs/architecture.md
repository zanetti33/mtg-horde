...
- `scripts/fetch-card-assets.mjs` extracts every `scryfallName` referenced in the two deck files, resolves each against the Scryfall API (same fuzzy-name lookup as `scryfall/api.ts`), **and** extracts every distinct *token* identity (name + power/toughness/keywords) from `CreateCreature` effects with `count > 1`, resolving each against Scryfall's real token cards (`layout:token` search, matched on power/toughness and, when more than one token shares that name/stat-line, an exact keyword match too — see the script's own comments). It writes:
  - `public/cards/<scryfallId>.jpg` — the card/token image, unmodified, served as a static asset.
  - `src/data/scryfallCache.store.json` — the persistent source of truth (queried key → resolved data; token entries use a `token:name|pow/tou|keywords` key, see `tokenCacheKeyFor`), so re-running the script only fetches entries not already in the store instead of re-downloading everything.
  - `src/data/scryfallCache.ts` — regenerated from the store on every run; exports `LOCAL_SCRYFALL_CACHE` (regular cards, keyed by lowercased/trimmed name, same as `cacheKeyFor` in `scryfall/api.ts`) and `LOCAL_TOKEN_SCRYFALL_CACHE` + `tokenCacheKeyFor` (tokens). Image URLs are resolved through `import.meta.env.BASE_URL` so they work both in dev (`/`) and on the GitHub Pages build (`/mtg-horde/`).
- `src/data/attachLocalScryfallData.ts` attaches a matching `LOCAL_SCRYFALL_CACHE` entry to each card's `.scryfall`, and — for a `CreateCreature` effect with `count > 1` and a `tokenName` — a matching `LOCAL_TOKEN_SCRYFALL_CACHE` entry to `effect.tokenScryfall`, when a preset deck module loads. Since both are already populated by the time `hydrateMissingScryfallData` / the `AppContext` startup effect run, neither issues a network request for them — the live API only gets hit for cards missing from the bundle (e.g. a card just added to a preset before the next `fetch-cards` run) or added through the deck builder (which has no token-art path yet, live or bundled).
- Run `npm run fetch-cards` after adding a new card (or a new token-producing entry) to a preset deck. It's safe to re-run any time — already-cached entries are skipped; pass `--force` to refresh everything (e.g. if Scryfall updates an image).
